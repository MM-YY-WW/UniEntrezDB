...
for i in range(1, EPOCHS+1):
    if i % VALIDATE_EVERY == 0:
        ...
        truths = []
        with torch.no_grad():
            for index, (data_v, labels_v) in tqdm(enumerate(val_loader), total=len(val_loader)):
                index += 1
            ...
            if is_master:
                print(f'    == Fold {args.current_fold} Epoch: {i} | cur_acc: {cur_acc:.6f} Validation Loss: {val_loss:.6f} | F1 Score: {f1:.6f}  ==')
                ...
                all_results.loc[all_results['Model_name'] == args.model_name[11:-5], f'SingleCell_Fold{args.current_fold}_Acc'] = round(cur_acc, 4)
                all_results.loc[all_results['Model_name'] == args.model_name[11:-5], f'SingleCell_Fold{args.current_fold}_F1'] = round(f1, 4)
                all_results.to_csv("results_overview.csv", sep='\t', header=True, index=False)
            
                logging.info(f'    ==  Epoch: {i} | cur_acc: {cur_acc:.6f} Validation Loss: {val_loss:.6f} | F1 Score: {f1:.6f}  ==')
