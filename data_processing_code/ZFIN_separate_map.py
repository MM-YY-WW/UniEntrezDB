...
combine = pd.read_csv("Go_annotation/extra_go/ZFIN/zfin.gaf", comment='!', names=columns16, sep='\t')
combine = combine[processed_columns]
combine = combine.drop_duplicates(keep='last')
combine.to_csv("Go_annotation/extra_go/ZFIN/combine.gaf", sep='\t', header=True, index=False)



print(len(combine))
combine = pd.read_csv("Go_annotation/extra_go/ZFIN/combine.gaf", sep='\t')
...
if not os.path.exists("Go_annotation/extra_go/ZFIN/unientrez_files"):
    os.makedirs("Go_annotation/extra_go/ZFIN/unientrez_files")
unientrez.to_csv("Go_annotation/extra_go/ZFIN/unientrez_files/uni_all.csv", sep='\t', header=True, index=False)
